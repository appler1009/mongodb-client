...
async function generateReleaseNotes() {
  ...
  try {
    ...
    if (numCommitsSinceLastTag === 0) {
      ...
      // This handles the initial release or releases with no changes.
      ...
      // This gets commits since the last tag AND the necessary number of older commits
      // to make it at least 10, ensuring sufficient context.
    ...
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 500, // IMPORTANT: Increased to avoid 'finish_reason: "length"'
        temperature: 0.7,
      }),
    });
    ...
    // Check if choices and message content exist before assigning
    const releaseNotes = data.choices && data.choices[0] && data.choices[0].message && typeof data.choices[0].message.content === 'string'
      ? data.choices[0].message.content
      : 'No release notes generated by API. Check API response structure.'; // More specific fallback
